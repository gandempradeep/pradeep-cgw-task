...
queryFieldsString;
    ...
    generateJson(data){
        ...
        if(data != null){
            this.allData = data;
            this.closeDate = data[this.invoice_date];
            this.dueDate = data[this.invoice_due_date];
            this.lineItemsMap = data[childrelationName].map(item => ({// Using square bracket notation instead of dot to handle dynamic nature of thr child object because it might be differ for different objects
                Description: item[this.line_item_description] || '',
                Quantity: item[this.line_item_quantity] || '0.0000',
                UnitAmount: item[this.line_item_unit_price] || '0.00'
            }))
            this.invoiceJSON = {
                Invoices: [
                    {
                        ...
                        DateString: data[this.invoice_date]+'T00:00:00' || '',
                        DueDateString: data[this.invoice_due_date]+'T00:00:00' || '',
    ...
    handleCreateInvoices() {
        ...
        this.lineItemsMap = this.allData[childrelationName].map(item => ({// Using square bracket notation instead of dot to handle dynamic nature of thr child object because it might be differ for different objects
                Line_Description__c: item[this.line_item_description] || '',
                Quantity__c: item[this.line_item_quantity] || '0.0000',
                Unit_Price__c: item[this.line_item_unit_price] || '0.00'
            }))
